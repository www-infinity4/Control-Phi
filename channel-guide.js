(function(){
  ...
  const CACHE_KEY='controlPhi:liveGuide:v3';
  ...
  const writeCache=value=>{try{localStorage.setItem(CACHE_KEY,JSON.stringify(value))}catch(_){}};

  function normalizeChannelMenu(){
    const menu=document.querySelector('details.channel-menu,details[data-channel-menu]');if(!menu)return;
    ...
    if(document.getElementById('controlPhiChannelShellStyles'))return;
  ...
  function currentProgram(doc=document){
    ...
    for(const selector of selectors){const value=clean(doc.querySelector(selector)?.textContent);if(valid(value))return value}return''
  }
  function currentImage(doc=document,base=location.href){
    const direct=doc.querySelector('[data-program-art] img,[data-now-art] img,.guide-row.current img,.row.now img,.current-program img')?.getAttribute?.('src')||'';if(direct)return safeUrl(direct,base);
    try{const raw=doc.body?.style?.getPropertyValue('--program-art')||doc.documentElement?.style?.getPropertyValue('--program-art')||'';const match=String(raw).match(/url\(["']?([^"')]+)["']?\)/i);if(match?.[1])return safeUrl(match[1],base)}catch(_){ }
  ...
  function mount(channels){
    ...
    const nav=section.querySelector('.icg-list'),cache=readCache();
    channels.filter(channel=>channel.path!=='News-Phi').forEach(channel=>{const link=document.createElement('a');link.className='icg-channel';link.href=ROOT+encodeURIComponent(channel.path).replace(/%2F/g,'/')+'/';link.dataset.path=channel.path;link.dataset.search=channel.name.toLowerCase();if(channel.path===pagePath)link.setAttribute('aria-current','page');const cached=cache[channel.path],program=channel.path===pagePath?currentProgram():(cached&&Date.now()-cached.at<300000?cached.program:''),image=channel.path===pagePath?currentImage(document,location.href):(cached?.image||'');link.innerHTML='<img class="icg-art" alt="" loading="lazy"><span><small>LIVE</small><strong></strong><em></em></span>';link.querySelector('strong').textContent=channel.name;link.querySelector('em').textContent=program||'Checking live schedule…';const img=link.querySelector('img');if(image){img.src=image;img.hidden=false}else img.hidden=true;nav.appendChild(link)});
  ...
  function probeOnDemand(nav,cache){
    ...
    function pump(){const max=innerWidth<=760?1:2;while(active<max&&queue.length){const link=queue.shift();if(!link||link.getAttribute('aria-current')==='page')continue;active++;probe(link).finally(()=>{active--;pump()})}}
    function probe(link){return new Promise(resolve=>{const frame=document.createElement('iframe');frame.className='icg-probe';frame.tabIndex=-1;frame.setAttribute('aria-hidden','true');let done=false,timer=0,limit=0;const expected=link.dataset.path.toLowerCase();const matches=()=>{try{return (frame.contentWindow.location.pathname.split('/').filter(Boolean)[0]||'').toLowerCase()===expected}catch{return false}};const finish=(program,image)=>{if(done)return;done=true;clearInterval(timer);clearTimeout(limit);frame.onload=null;frame.remove();const label=link.querySelector('em'),img=link.querySelector('img');if(valid(program)){label.textContent=program;link.dataset.search+=' '+program.toLowerCase()}else label.textContent='Open live schedule';if(image){img.src=image;img.hidden=false}cache[link.dataset.path]={program:valid(program)?program:'',image:image||'',at:Date.now()};writeCache(cache);resolve()};const inspect=()=>{try{if(!matches())return;const doc=frame.contentDocument,title=currentProgram(doc),image=currentImage(doc,link.href);if(title)finish(title,image)}catch(_){ }};frame.onload=()=>{inspect();timer=setInterval(inspect,250)};limit=setTimeout(()=>{let title='',image='';try{if(matches()){title=currentProgram(frame.contentDocument);image=currentImage(frame.contentDocument,link.href)}}catch(_){ }finish(title,image)},5000);frame.src=link.href+'?omni=meta&guide=1&v='+Date.now();document.body.appendChild(frame)})}
    const observer=new IntersectionObserver(entries=>{entries.forEach(entry=>{const link=entry.target;if(entry.isIntersecting&&!queued.has(link)){queued.add(link);queue.push(link);observer.unobserve(link)}});pump()},{rootMargin:'300px'});nav.querySelectorAll('.icg-channel').forEach(link=>observer.observe(link))
  }
